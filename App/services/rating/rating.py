...
- **EXPLICIT MENTION** of GAP-related terms: "GAP", "GAP Insurance", "GAP Coverage", "Guaranteed Auto Protection"
- **AND** a specific price > $0 is clearly associated with it
- **AND** the price is not $0 or missing

**If GAP is NOT explicitly mentioned OR price is $0/missing → GAP is ABSENT**
- **NO GREEN FLAG** for fair pricing (because it's not there to be fair)
- **NO AUDIT** of GAP pricing rules
- **ONLY** apply missing GAP penalty if conditions met ($0 down AND 75+ month term)

**VSC Detection - MUST FOLLOW EXACTLY:**
VSC is ONLY considered present when:
- **EXPLICIT MENTION** of VSC-related terms: "VSC", "Extended Warranty", "Service Contract", "Protection Plan", "Warranty Coverage"  
- **AND** a specific price > $0 is clearly associated with it
- **AND** the price is not $0 or missing

**If VSC is NOT explicitly mentioned OR price is $0/missing → VSC is ABSENT**
- **NO GREEN FLAG** for fair pricing (because it's not there to be fair)
- **NO AUDIT** of VSC pricing rules
- **ONLY** apply advisory note if high-mileage long-term loan

### STRICT INTERPRETATION FOR THE GIVEN DOCUMENT:

Looking at your document, I see:
- **NO MENTION** of "GAP", "GAP Insurance", "GAP Coverage", etc.
- **NO MENTION** of "VSC", "Extended Warranty", "Service Contract", etc.
- **NO PRICING** for GAP or VSC products

**THEREFORE:**
- GAP = ABSENT (not $0)
- VSC = ABSENT (not $0) 
- **NO GAP/VSC pricing audits should be performed**
- **NO GREEN FLAGS for GAP/VSC fair pricing**
- **ONLY** check missing GAP penalty rule ($0 down + 75+ month term)
...
### GREEN FLAG DETECTION AND OUTPUT REQUIREMENTS

**YOU MUST OUTPUT GREEN FLAGS WHEN PRESENT - DO NOT OMIT POSITIVE FINDINGS**

**GREEN FLAG DETECTION RULES - ENFORCE STRICTLY:**

1. **Transparent Pricing Display** → GREEN FLAG
   - Condition: Document clearly shows itemized breakdown (MSRP, rebates, fees, taxes, etc.)
   - Message: "Transparent pricing with clear itemization of all costs and fees"
   - Bonus: 0 (listing only)

2. **Substantial Down Payment** → GREEN FLAG  
   - Condition: Down payment ≥ $5,000 OR ≥ 20% of selling price
   - Message: "Substantial down payment of $[amount] reduces financial risk"
   - Bonus: 0 (listing only)

3. **Manufacturer Rebate Pass-through** → GREEN FLAG
   - Condition: Manufacturer rebate clearly listed and applied to reduce price
   - Message: "Manufacturer rebate of $[amount] passed through to buyer"
   - Bonus: 0 (listing only)

4. **Competitive APR** → GREEN FLAG
   - Condition: APR ≤ 9.5% on dealer-financed deals
   - Message: "Competitive APR of [rate]%"
   - Bonus: As specified in APR bonus rules

**GREEN FLAG OUTPUT ENFORCEMENT:**
- You MUST actively look for and list ALL applicable green flags
- DO NOT skip green flags just because they don't add points
- Every positive aspect of the deal MUST be captured in green_flags array
- Minimum green flag count for fair deals: 1-2 flags
- Minimum green flag count for good deals: 3+ flags
...
"green_flags": {
  ...
  "scoring_effect": "Each green flag may increase the score. But some may not."
},
...
□ If GAP/VSC not explicitly mentioned with price > $0 → NO green flags for them
□ Only apply GAP missing penalty if $0 down AND 75+ month term
□ VSC missing is always blue flag (advisory only)
